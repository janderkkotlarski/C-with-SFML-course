...
int main()
{
	...
	const std::string programmanaam = "Simpel Platform Spel V1.0";
	...
	for (int a_sub = 0; a_sub <= 10; a_sub++)
	{
		...
		if (a_sub == 10)
		{
			...
			your_finish.set_coords(halve_window_x + (10 + 3*a_sub + 6)*x_img, halve_window_y + (-a_sub + 5 - 3)*y_img);
	...
	for (int a_sub = 0; a_sub <= 20; a_sub++)
	{
		...
		if (a_sub < 10)
		{
			...
			your_wall.resize(++wall_amount);
			your_wall[wall_amount - 1].set_coords(halve_window_x + 1*x_img, halve_window_y + (-a_sub + 3)*y_img);
