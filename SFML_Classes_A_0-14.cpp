...
int main()
{
	...
	const std::string programmanaam = "SFML Classes A V0.11";
	...
	const int char_x_pix = 48, char_y_pix = 48;
	...
	const std::string char_img = "Farbstract_48.png";
	
	// Bestandsnaam van de muursprite
	const std::string wall_img = "Chromatoid_64.png";
	...
	your_chara.set_steps(x_move, y_move);
	...
	const double chara_ddx = 0, chara_ddy = 0.25;
	...
	your_wall[wall_amount - 1].set_coords(halve_window_x + 0*x_img, halve_window_y + 6*y_img);

	your_wall.resize(++wall_amount);
	your_wall[wall_amount - 1].set_coords(halve_window_x - 3*x_img, halve_window_y);
	
	your_wall.resize(++wall_amount);
	your_wall[wall_amount - 1].set_coords(halve_window_x + 4*x_img, halve_window_y + 4*y_img);
	
	your_wall.resize(++wall_amount);
	your_wall[wall_amount - 1].set_coords(halve_window_x, halve_window_y + 2*y_img);	
	...
	for (int a_sub = 0; a_sub < 10; a_sub++)
	{
		...
		your_wall.resize(++wall_amount);
		your_wall[wall_amount - 1].set_coords(halve_window_x + (-5 + a_sub)*x_img, halve_window_y + 5*y_img);
	...
	for (int a_sub = 0; a_sub < wall_amount; a_sub++)
	{
		...
		your_wall[a_sub].set_steps(x_move, y_move);
	...
	while (window.isOpen())
    {
		...
		your_chara.set_steps(x_move, chara_dy);
